...
[image1]: ./examples/data-set-vis-random-15 "Visualization"
...
Here is an exploratory visualization of the data set. The first image shows a matrix of randomly selected images with classes obtained from the provided description in ``signames.csv``. The next images depict the distribution of classes in the respective data sets as bar charts.

![15 randomly selected images without pre-processing][data-set-vis-random-15.png]
![Distribution of classes in the training data set][data-set-vis-freq-classes-training.png]
![Distribution of classes in the validation data set][data-set-vis-freq-classes-valid.png]
![Distribution of classes in the test data set][data-set-vis-freq-classes-test.png]
...
Here is an example of the explorative data set after grayscaling and after normalization (without grayscaling). 

![Explorative data set after grayscaling][pre-processing-grayscale.png]
         
![Explorative data set after normalization][pre-processing-normalization.png]

The function ``pre_process`` represents a unified gateway to pre-processing. It turned out during the experimentation that simple zero mean normalization had best results. Grayscale conversion made the training much faster (obviously, since the data set size was considerably reduced) but less stable - the convergence was slower, required more epochs and the training curve experienced unpredictable oscillations.

I did not generate additional data because the network produced good results with existing data. 
